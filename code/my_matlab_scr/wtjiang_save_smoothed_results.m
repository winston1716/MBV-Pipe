function wtjiang_save_smoothed_results(T2_vbm_dartel_dir, saved_dir)
    split_dir_str = strsplit(T2_vbm_dartel_dir, '/');
    sub_id = split_dir_str(end-1);
    t_states = {'invivo', 'perfused', 'fixed'};
    invivo_results_fname = ['T2_', t_states{1}, '_smoothed_volume_stats.mat'];
    invivo_results_fpath = fullfile(T2_vbm_dartel_dir, invivo_results_fname);
    invivo_results_mat = load(invivo_results_fpath);
    invivo_results_data = invivo_results_mat.volume_stats_results;
    
    perfused_results_fname = ['T2_', t_states{2}, '_smoothed_volume_stats.mat'];
    perfused_results_fpath = fullfile(T2_vbm_dartel_dir, perfused_results_fname);
    perfused_results_mat = load(perfused_results_fpath);
    perfused_results_data = perfused_results_mat.volume_stats_results;
    
    fixed_results_fname = ['T2_', t_states{3}, '_smoothed_volume_stats.mat'];
    fixed_results_fpath = fullfile(T2_vbm_dartel_dir, fixed_results_fname);
    ...
    saved_fig_fname = strcat(sub_id, '_smoothed_results.png');
